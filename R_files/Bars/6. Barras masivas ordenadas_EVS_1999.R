...
# working directory.
dataset_name <- "ZA3811_v3-0-0.sav"
unzip(zipfile = "SAV_files/EVS/datasets/EVS_80-08.zip", 
      exdir = "SAV_files/", list = TRUE)
unzip(zipfile = "SAV_files/EVS/datasets/EVS_80-08.zip", 
      exdir = "SAV_files/", list = FALSE, files = dataset_name) 
# 1.3 Read the SPSS data into R.
mydata <- spss.system.file("SAV_files/ZA3811_v3-0-0.sav")
# 1.4 Subset the relevant World Values Survey questions into our data frame. For 
# example, these include the country (v2, v2a), the thematic 
# questions (v45-v54) and the socio demographic question about 
# the person's sex (v240).
# mydata <- subset(mydata,
#                    select = c(country, v45, v47, v48, v50, v51, v52, v53, v54, 
#                               v240))
# 1.4.1 Retrieve the country names to know which ones were surveyed in this 
# wave/dataset.
# countries1 <- names((table(mydata$v2)))
# countries1 # Display the countries available 
# 1.5 Subset the data for a selected country.
# mydata <- subset(mydata, v2 == "Zimbabwe")
...
# 1.4 Subset the relevant European Values Survey questions into our data frame. 
# These include the country (country, the thematic questions (v45-v54) and 
# the socio demographic question about the person's sex (v240).
# 1999 wave: 
# v149: Women need children in order to be fulfilled.
# v154: Working mother warm relationship with children
# v157: Being a housewive is as fulfilling as paid job
# v157_5c: Being a housewive is as fulfilling as paid job (5 answer categories).
# v159: Husband and wife should both contribute to income.
# v159_5c: Husband and wife should both contribute to income.
# v161_se: Men make better political leaders than women do.
# v162_se: University is more important for a boy than for a girl.
# v155 or v155_5c: Pre-school child suffers with working mother.
# v156: Women want a home and children.
# v156c: Women really want home and children (5 answer categories)
# v158: Job best way for women to be independent
# v158_5c: Job best way for women to be independent (5 answer categories)
# v160 Fathers as well suited to look after children as mothers.
# v160_5c Fathers as well suited to look after children as mothers.
# v161: Men less able to handle emotions in relationships than women.
# v161_5c: Men less able to handle emotions in relationships than women 
#                                                       (5 answer categories).
mydata <- subset(mydata,
                 select = c(country, year, v157, v155, v158, v156, 
                            v160, v161, v154, v291))
countries1 <- names((table(mydata$country)))
mydata <- subset(mydata, country == "Finland")
country <- names(table(mydata$country))
year <- names(table(mydata$year))
mydata$country <- NULL 
mydata$year <- NULL
...
######## STEP 4. PLOT CHART ##############################################
# Prepare chart
pdf_file <- "PDF_files/Women_finland.pdf"
...
mtext(paste("Opinions about gender roles in ", country, ", ", year, sep = ""),
      3, line = -3, adj = 0,
...
mtext("M", 1, line = -0.5, adj = 0.1, cex = 2.5, outer = TRUE, font = 3, 
      col = col_m)
mtext( paste("Source: European Values Survey, ", year,". \n Country: ", 
             country, ".\n Dataset: ", dataset_name, sep = ""), 1, 
