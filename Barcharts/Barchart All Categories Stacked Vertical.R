...
inch <- 2.54
fileName <- paste("PDF_files/",selectedCountry,"_", year,"_all_stacked_v.jpg", 
                  sep = "")
#cairo_pdf(filename = fileName, width = 27.94/inch, height = 21.59/inch, 
 #         bg = "grey98")
jpeg(filename = fileName, width = 27.94/inch, height = 21.59/inch, 
          bg = "grey98", quality = 100, units = "in", res = 400)
par(omi = c(2/inch, 1/inch, 2/inch, 1.5/inch), 
    mai = c(4/inch, 1/inch, 3/inch, 6/inch), family = "Lato Light", las = 1,
    mgp = c(3, 3, 0))
# 3.2 Define measures and colors of the barplot---------------------------------
ylength <- 100
xlength <- n_sections
barwidth <- 0.50
valuespace <- 0.80/barwidth
...
bp <- barplot(t(as.matrix(fData[, 2:ncol(fData)])), names.arg = barLabels, 
              cex.names = 0.65, horiz = FALSE, border = NA, xlim = c(0, xlength), 
              ylim = c(0, ylength), col = colors, axes = FALSE, family = "Lato Light", 
              width = barwidth, space = valuespace)
# 3.4 Set value labels for each bar---------------------------------------------
ypos <- apply(t(as.matrix(fData[ , 2:ncol(fData)])), 2, cumsum)
ypos <- ypos - t(as.matrix(fData[ , 2:ncol(fData)])) / 2
ypos <- t(ypos)
text(bp, ypos, 
     format(round(as.matrix(fData[, 2:ncol(fData)]), 1), nsmall = 1), 
     cex = 0.75, col = "gray27", xpd = TRUE) 
# 3.5 Draw responses labels-------------------------------------------
px <-  c(1, 2.905, 4.850, 6.350, 7.900)
py <- rep(ylength*1.05, length(responses))
points(px, py, pch = 15, cex = 2, col = colors, xpd = TRUE)
tx <- px + 0.2
ty <- rep(ylength*1.05, length(responses))
...
mtext(text = chartSubtitle, side =  3, line = -2, adj = 0, cex = 1.5, 
      outer = TRUE, font = 3)
mtext(c(0, 20, 40, 60, 80, 100), at = c(0, 20, 40, 60, 80, 100), side = 2, 
      line = -4, cex = 0.75)
mtext(text = scaleTag, at = xlength*1.3, line = 0, side = 1, cex = 0.9, adj = 1, font = 3)
mtext(text = sampleTag, at = xlength*1.4, side = 1, line = 6, adj = 1, cex = 0.9, font = 3)
mtext(text =  sourceTag, at = xlength*1.4, side = 1, line = 5, adj = 1, cex = 0.9, font = 3)
mtext(text =  dateTag, at = xlength*1.4, side = 1, line = 7, adj = 1, cex = 0.9, font = 3)
dev.off() 